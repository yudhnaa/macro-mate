...
class OpenRouterVLM(BaseVisionLanguageModel):
    ...
    def _generate(
        ...
        try:
            ...
            data = response.json()
            logger.info(f"🔵 OpenRouter full response: {json.dumps(data, indent=2, ensure_ascii=False)}")
            ...
            if isinstance(content, list):
                ...
                for part in content:
                    ...
                    # ✅ Xử lý định dạng image từ LangChain
                         ...
                         converted.append(part)
                    elif part.get("type") == "image": # Đây là định dạng bạn dùng trong vision_node
                        if part.get("source_type") == "url":
                            converted.append({
                                "type": "image_url",
                                "image_url": {"url": part["url"]}
